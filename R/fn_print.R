    ...
    toy_data_dv_1L_chr = "fakes", what_1L_chr = "all", ...) 
{
    dvs_tb <- add_references(dvs_tb, data_var_nm_1L_chr = "Contents", 
        ...
        "Name", "Description", "Creator", "Datasets") %>% dplyr::mutate(Datasets = .data$Datasets %>% 
    ...
    what_chr = "all", ...) 
{
    ...
    ready4_badges_chr <- purrr::map_chr(pkg_extensions_tb$Badges, 
        ~{
            ...
            ifelse(identical(badge_1L_chr, character(0)), NA_character_, 
                badge_1L_chr)
        })
    ...
    what_chr = "all", ...) 
{
    ...
    vignettes_tb <- tibble::tibble(HTML = vignettes_chr[keep_lgl]) %>% 
        dplyr::mutate(Title = purrr::map_chr(.data$HTML, ~rvest::read_html(.x) %>% 
            rvest::html_elements("h1") %>% rvest::html_text2()), 
            RMD = purrr::map_chr(.data$HTML, ~rvest::read_html(.x) %>% 
                rvest::html_elements(".dont-index") %>% rvest::html_elements("a") %>% 
                rvest::html_attr("href"))) %>% dplyr::mutate(Program = purrr::map2(.data$HTML, 
        .data$RMD, ~kableExtra::cell_spec(c("HTML", "RMD"), "html", 
            link = c(.x, .y))))
