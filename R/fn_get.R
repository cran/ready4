#' Get badge urls
#' @description get_badge_urls() is a Get function that extracts data from an object. Specifically, this function implements an algorithm to get badge urls. The function returns Badge urls (a list).
...
{
    images_xx <- get_gracefully(paste0(url_stub_1L_chr, pkg_nm_1L_chr, 
        "/index.html"), fn = rvest::read_html, not_chr_1L_lgl = TRUE)
    if (!is.null(images_xx)) {
        images_chr <- images_xx %>% rvest::html_elements("img") %>% 
            rvest::html_attr("src")
...
#' @description get_badges_lup() is a Get function that extracts data from an object. Specifically, this function implements an algorithm to get badges lookup table. The function returns Ready4 badges (a lookup table).
    ...
    gh_tag_1L_chr = "Documentation_0.0") 
{
    ...
    if (!is.null(dmt_urls_xx)) {
        ...
        ready4_badges_lup <- get_gracefully(dmt_urls_chr[dmt_urls_chr %>% 
            endsWith(ends_with_1L_chr)], not_chr_1L_lgl = TRUE)
    }
    return(ready4_badges_lup)
}
#' Get class extensions
#' @description get_cls_extensions() is a Get function that extracts data from an object. Specifically, this function implements an algorithm to get class extensions. The function returns Class extensions (a tibble).
    ...
    validate_1L_lgl = FALSE) 
{
    ...
    if (!is.null(dmt_urls_xx)) {
        ...
        cls_extensions_tb <- get_gracefully(dmt_urls_chr[dmt_urls_chr %>% 
            endsWith("classes_lup.RDS")], not_chr_1L_lgl = TRUE)
    }
    if (!is.null(cls_extensions_tb)) {
        cls_extensions_tb <- cls_extensions_tb %>% tibble::as_tibble() %>% 
            dplyr::arrange(.data$pt_ns_chr) %>% dplyr::filter(.data$pt_ns_chr %in% 
            pkg_extensions_tb$pt_ns_chr) %>% dplyr::arrange(.data$pt_ns_chr) %>% 
        ...
        if (validate_1L_lgl) {
            cls_extensions_tb <- cls_extensions_tb$pt_ns_chr %>% 
                ...
                if (!is.null(allowable_xx)) {
                  allowable_chr <- allowable_xx %>% rvest::html_elements("a") %>% 
                    rvest::html_text2()
                  allowable_chr <- allowable_chr[(allowable_chr %>% 
                    ...
                    "")
                  cls_extensions_tb %>% dplyr::filter(.data$pt_ns_chr == 
                    .x) %>% dplyr::filter(.data$type_chr %in% 
                    allowable_chr)
                }
                else {
                  NULL
...
#' Get datasets tibble
#' @description get_datasets_tb() is a Get function that extracts data from an object. Specifically, this function implements an algorithm to get datasets tibble. The function returns Datasets (a tibble).
#' @param gh_repo_1L_chr Github repository (a character vector of length one), Default: 'ready4-dev/ready4'
#' @param gh_tag_1L_chr Github tag (a character vector of length one), Default: 'Documentation_0.0'
    ...
    rds_fl_name_1L_chr = "datasets_tb") 
{
    ...
    if (!is.null(dmt_urls_xx)) {
        dmt_urls_chr <- dmt_urls_xx
        datasets_tb <- get_gracefully(dmt_urls_chr[dmt_urls_chr %>% 
            endsWith(paste0(rds_fl_name_1L_chr, ".RDS"))], not_chr_1L_lgl = TRUE)
    }
    return(datasets_tb)
}
#' Get digits from text
#' @description get_digits_from_text() is a Get function that extracts data from an object. Specifically, this function implements an algorithm to get digits from text. The function returns Digits (a character vector).
...
#' @description get_dv_fls_urls() is a Get function that extracts data from an object. Specifically, this function implements an algorithm to get dataverse files urls. The function returns Urls (a character vector).
...
#' Get examples
#' @description get_examples() is a Get function that extracts data from an object. Specifically, this function implements an algorithm to get examples. The function returns Examples (a character vector).
...
get_examples <- function (vignettes_chr, term_1L_chr) 
{
    if (is.na(vignettes_chr[1])) {
        ...
        examples_chr <- vignettes_chr %>% purrr::map_chr(~{
            code_xx <- get_gracefully((.x %>% stringr::str_match("href=\"\\s*(.*?)\\s*\" style"))[, 
                2], fn = rvest::read_html, not_chr_1L_lgl = TRUE)
            if (!is.null(code_xx)) {
                code_chr <- code_xx %>% rvest::html_elements(".r") %>% 
                  rvest::html_text2()
...
#' @description get_excluded_repos() is a Get function that extracts data from an object. Specifically, this function implements an algorithm to get excluded repositories. The function returns Exclude (a character vector).
#' @param gh_repo_1L_chr Github repository (a character vector of length one), Default: 'ready4-dev/ready4'
#' @param gh_tag_1L_chr Github tag (a character vector of length one), Default: 'Documentation_0.0'
...
#' @description get_functions_tb() is a Get function that extracts data from an object. Specifically, this function implements an algorithm to get functions tibble. The function returns Functions (a tibble).
#' @param gh_repo_1L_chr Github repository (a character vector of length one), Default: 'ready4-dev/ready4'
#' @param gh_tag_1L_chr Github tag (a character vector of length one), Default: 'Documentation_0.0'
    ...
    return_1L_chr = "all") 
{
    ...
    if (!is.null(dmt_urls_xx)) {
        ...
        functions_tb <- get_gracefully(dmt_urls_chr[dmt_urls_chr %>% 
            endsWith("fn_types_lup.RDS")], not_chr_1L_lgl = TRUE)
        if (!is.null(functions_tb)) {
            if (return_1L_chr == "methods") {
                functions_tb <- functions_tb %>% dplyr::filter(.data$is_method_lgl)
            }
...
#' Get generics
#' @description get_generics() is a Get function that extracts data from an object. Specifically, this function implements an algorithm to get generics. The function returns Generics (a character vector).
...
#' @description get_gracefully() attempts to retrieve objects from the internet but returns NULL and an informative message if there is no internet connection or the specified resource could not be found.
#' @param url_1L_chr Url (a character vector of length one)
#' @param args_ls Arguments (a list), Default: NULL
#' @param fn Function (a function), Default: readRDS
#' @param not_chr_1L_lgl Not character vector (a logical vector of length one), Default: 'NA'
#' @param tests_chr Tests (a character vector), Default: character(0)
#' @return Object (an output object of multiple potential types)
#' @rdname get_gracefully
#' @export 
...
#' }
get_gracefully <- function (url_1L_chr, args_ls = NULL, fn = readRDS, not_chr_1L_lgl = NA_character_, 
    tests_chr = character(0)) 
{
    ...
    if (!curl::has_internet()) {
        ...
        if (!is.null(url_xx) | is.null(url_1L_chr)) {
            object_xx <- suppressWarnings(tryCatch(tryCatch(rlang::exec(fn, 
                url_xx, !!!args_ls)), error = function(e) conditionMessage(e)))
            if (is.character(object_xx)) {
                if (any(tests_chr %>% purrr::map_lgl(~stringr::str_detect(object_xx[1], 
                  .x))) | not_chr_1L_lgl) {
                  message(object_xx)
                  object_xx <- invisible(NULL)
                }
...
#' @description get_libraries_tb() retrieves a tabular summary of ready4 libraries that have been developed within a specified GitHub organisation.
#' @param gh_repo_1L_chr Github repository (a character vector of length one), Default: 'ready4-dev/ready4'
#' @param gh_tag_1L_chr Github tag (a character vector of length one), Default: 'Documentation_0.0'
...
get_libraries_tb <- function (gh_repo_1L_chr = "ready4-dev/ready4", gh_tag_1L_chr = "Documentation_0.0") 
{
    ...
    if (!is.null(dmt_urls_xx)) {
        ...
        if (any(dmt_urls_chr %>% endsWith("libraries_tb.RDS"))) {
            libraries_tb <- get_gracefully(dmt_urls_chr[dmt_urls_chr %>% 
                endsWith("libraries_tb.RDS")], not_chr_1L_lgl = TRUE)
        }
    }
    return(libraries_tb)
}
#' Get manual urls
#' @description get_manual_urls() is a Get function that extracts data from an object. Specifically, this function implements an algorithm to get manual urls. The function returns Urls (a character vector).
#' @param pkg_nm_1L_chr Package name (a character vector of length one), Default: 'ready4'
...
get_manual_urls <- function (pkg_nm_1L_chr = "ready4", pkg_url_1L_chr = "https://ready4-dev.github.io/ready4/index.html") 
{
    urls_xx <- get_gracefully(pkg_url_1L_chr, fn = rvest::read_html, 
        not_chr_1L_lgl = TRUE)
    if (!is.null(urls_xx)) {
        urls_chr <- urls_xx %>% rvest::html_elements(".external-link") %>% 
            rvest::html_attr("href")
        indcs_int <- urls_chr %>% purrr::map_lgl(~endsWith(.x, 
            paste0(pkg_nm_1L_chr, "_User.pdf")) | endsWith(.x, 
            paste0(pkg_nm_1L_chr, "_Developer.pdf"))) %>% which()
...
#' @description get_methods() retrieves the ready4 methods that are available for a specified ready4 model module.
#' @param pkg_nm_1L_chr Package name (a character vector of length one), Default: 'ready4'
...
#' @description get_methods_tb() ingests 'methods_tb.RDS' (a table of methods associated with ready4 model modules) from a specified GitHub repository release.
#' @param gh_repo_1L_chr Github repository (a character vector of length one), Default: 'ready4-dev/ready4'
#' @param gh_tag_1L_chr Github tag (a character vector of length one), Default: 'Documentation_0.0'
...
get_methods_tb <- function (gh_repo_1L_chr = "ready4-dev/ready4", gh_tag_1L_chr = "Documentation_0.0") 
{
    ...
    if (!is.null(dmt_urls_xx)) {
        dmt_urls_chr <- dmt_urls_xx
        methods_tb <- get_gracefully(dmt_urls_chr[dmt_urls_chr %>% 
            endsWith("methods_tb.RDS")], not_chr_1L_lgl = TRUE)
    }
    return(methods_tb)
}
#' Get a table of ready4 model modules
#' @description get_modules_tb() ingests 'modules_tb.RDS' (a table of ready4 model modules) from a specified GitHub repository release.
#' @param gh_repo_1L_chr Github repository (a character vector of length one), Default: 'ready4-dev/ready4'
#' @param gh_tag_1L_chr Github tag (a character vector of length one), Default: 'Documentation_0.0'
...
get_modules_tb <- function (gh_repo_1L_chr = "ready4-dev/ready4", gh_tag_1L_chr = "Documentation_0.0") 
{
    ...
    if (!is.null(dmt_urls_xx)) {
        dmt_urls_chr <- dmt_urls_xx
        modules_tb <- get_gracefully(dmt_urls_chr[dmt_urls_chr %>% 
            endsWith("modules_tb.RDS")], not_chr_1L_lgl = TRUE)
    }
    return(modules_tb)
}
#' Get method titles
#' @description get_mthd_titles() is a Get function that extracts data from an object. Specifically, this function implements an algorithm to get method titles. The function returns Method titles (a character vector).
...
#' @description get_r4_obj_slots() is a Get function that extracts data from an object. Specifically, this function implements an algorithm to get ready4 module object slots. The function returns Slots (a character vector).
...
#' @description get_rds_from_dv() is a Get function that extracts data from an object. Specifically, this function implements an algorithm to get rds from dataverse. The function returns R object (an output object of multiple potential types).
#' @param file_nm_1L_chr File name (a character vector of length one)
#' @param dv_ds_nm_1L_chr Dataverse dataset name (a character vector of length one), Default: 'https://doi.org/10.7910/DVN/RIQTKK'
#' @param dv_url_pfx_1L_chr Dataverse url prefix (a character vector of length one), Default: character(0)
#' @param key_1L_chr Key (a character vector of length one), Default: NULL
#' @param not_chr_1L_lgl Not character vector (a logical vector of length one), Default: NA
#' @param server_1L_chr Server (a character vector of length one), Default: Sys.getenv("DATAVERSE_SERVER")
...
get_rds_from_dv <- function (file_nm_1L_chr, dv_ds_nm_1L_chr = "https://doi.org/10.7910/DVN/RIQTKK", 
    dv_url_pfx_1L_chr = character(0), key_1L_chr = NULL, not_chr_1L_lgl = NA, 
    server_1L_chr = Sys.getenv("DATAVERSE_SERVER")) 
{
    ...
    if (identical(idx_1L_int, integer(0))) {
        ...
        r_object_xx <- get_gracefully(paste0(dv_url_pfx_1L_chr, 
            ds_ls[[idx_1L_int]]$dataFile$id), not_chr_1L_lgl = not_chr_1L_lgl)
    }
    return(r_object_xx)
}
#' Get source code urls
#' @description get_source_code_urls() is a Get function that extracts data from an object. Specifically, this function implements an algorithm to get source code urls. The function returns Urls (a character vector).
#' @param pkg_nm_1L_chr Package name (a character vector of length one), Default: 'ready4'
#' @param pkg_url_1L_chr Package url (a character vector of length one), Default: 'https://ready4-dev.github.io/ready4/index.html'
#' @return Urls (a character vector)
...
get_source_code_urls <- function (pkg_nm_1L_chr = "ready4", pkg_url_1L_chr = "https://ready4-dev.github.io/ready4/index.html") 
{
    ...
    if (!is.null(urls_xx)) {
        urls_chr <- urls_xx %>% rvest::html_elements(".external-link") %>% 
            rvest::html_attr("href")
        indcs_int <- c(which((urls_xx %>% rvest::html_elements(".external-link") %>% 
            rvest::html_text()) == "Browse source code"), which(startsWith(urls_chr, 
...
#' @description get_subroutine_repos() is a Get function that extracts data from an object. Specifically, this function implements an algorithm to get subroutine repositories. The function returns Subroutine repositories (a character vector).
#' @param gh_repo_1L_chr Github repository (a character vector of length one), Default: 'ready4-dev/ready4'
#' @param gh_tag_1L_chr Github tag (a character vector of length one), Default: 'Documentation_0.0'
...
get_subroutine_repos <- function (gh_repo_1L_chr = "ready4-dev/ready4", gh_tag_1L_chr = "Documentation_0.0") 
{
    ...
    if (!is.null(dmt_urls_xx)) {
        ...
        if (any(dmt_urls_chr %>% endsWith("subroutine_repos_chr.RDS"))) {
            subroutine_repos_chr <- get_gracefully(dmt_urls_chr[dmt_urls_chr %>% 
                endsWith("subroutine_repos_chr.RDS")], not_chr_1L_lgl = FALSE)
        }
        else {
    ...
    heading_rows_1L_int = 1L) 
{
    ...
    if (force_numeric_1L_lgl) {
        table_xx <- table_xx %>% dplyr::mutate(dplyr::across(tidyselect::where(is.character), 
